...
fn create_archive_with_index(src_dir: impl AsRef<Path>, output_zst: impl AsRef<Path>, dict: impl AsRef<Path>) -> Result<()> {
    ...
    let dict = File::open(dict)?.bytes().filter_map(Result::ok).collect::<Vec<_>>();
    ...
    for entry in WalkDir::new(&src_dir).into_iter().filter_map(|e| e.ok()) {
        if entry.file_type().is_file() {
            if !entry.file_name().to_str().map(|s| s.ends_with(".svg")).unwrap_or_default() {
                continue
            }
            let path = entry.path();
    ...
    for path in paths.iter() {
        ...
        let mut f = OpenOptions::new().read(true).write(false).create(false).open(&src_dir.as_ref().join(path))?;
        f.read_to_end(&mut buf)?;
        let uncompressed_size = buf.len();
        let mut obuf = Vec::with_capacity(2048);
        compressor.compress_using_cdict(&mut obuf, &buf, &dict).map_err(|s| anyhow!("could not compress : {}", zstd_safe::get_error_name(s)))?;
        let frame_header = obuf[4]; // 1byte frame header descriptor
        assert!(frame_header == 99 || frame_header == 35, "unexpected zstd frame header:{} {}", frame_header,  format!("{:?}", obuf));
        ...
        let fcs_field_size = frame_header >> 6; //
            ...
            // 3 => 8,
            // _ => 0,
        assert!(fcs_field_size == 0 || fcs_field_size == 1, "too large icon :{path} ( size > 65791 )");
        assert!(frame_header >> 5 & 1 == 1 , "not single segment ( probably too large file )");
        ...
        assert_eq!(dict_id, dict_id_written, "dict id mismatch : {dict_id_written}");
        ...
        let content_size = match fcs_field_size {
            ...
            _ => { panic!("icon {path} is too big") } // should not happen for small icons
        };
...
fn rebuild_dict(src_dir: impl AsRef<Path>, dict: impl AsRef<Path>) -> Result<()> {
    std::process::Command::new("bash")
        .args(&["-c", &format!("zstd --train --maxdict=2048 -o {} {}/*/*.svg", dict.as_ref().display(), src_dir.as_ref().display())]).spawn()?.wait()?;
    Ok(())
}
...
fn write_index_tree(mut index_file: impl Write + Seek, map: &Map) -> Result<usize> {
    ...
    for key in keys {
        ...
        debug!("writing value of {key} : {v} @ {}", index_file.stream_position()?);
...
fn trim_svg_tags(src_dir: &str) -> Result<()> {
    for entry in WalkDir::new(src_dir).into_iter().filter_map(|e| e.ok()) {
        if !entry.file_type().is_file() || 
            !entry.file_name().to_str().map(|s| s.ends_with(".svg")).unwrap_or_default() { continue }
        let path = entry.path();
        // let relative_path = path.strip_prefix(src_dir)?.to_string_lossy().into_owned();
