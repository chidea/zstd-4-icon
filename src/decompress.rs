...
fn get_raw_bytes_by_path(
    decompressor: &mut DCtx,
    dict_id: u32,
...
fn mock_read_frame(
    ...
    let _out_size = if header == 35 {
        ...
        return Err(anyhow!("unexpected frame header : {}", header));
    };
    let size = {
        let mut buf = [0u8; 3];
        archive_file.read_exact(&mut buf)?;
...
fn get_frame_pos(index_file: &mut (impl Read + Seek), key: &str) -> Result<u32> {
    ...
    for _step in 0..32 {
        // enough step to search 32bit unsigned hashes
        if high < low {
            break;
        }
...
pub fn new_dict_decompressor<'a>(
    dict_path: impl AsRef<Path>,
) -> Result<(DCtx<'a>, zstd_safe::DDict<'static>, u32)> {
    ...
    let dict_id = u32::from_le_bytes(dict[4..8].try_into().expect("dictionary id parse error"));
    ...
    decompressor.ref_ddict(&ddict).map_err(|s| {
        anyhow!(
            "could not load dictionary : {}",
            zstd_safe::get_error_name(s)
        )
    })?;
    Ok((decompressor, ddict, dict_id))
}
pub fn new_decompressor_wrapper<'a>(
    dict_path: impl AsRef<Path>,
) -> Result<(DecompressorWrapper<'a>, zstd_safe::DDict<'static>)> {
...
pub fn decompress_with_decompressor_wrapper<'a>(
    wrapper: &mut DecompressorWrapper<'a>,
...
pub fn decompress_with_dict_decompressor<'a>(
    decompressor: &mut DCtx<'a>,
    dict_id: u32,
    writer: &mut impl std::io::Write,
    archive: impl AsRef<Path>,
    key: &str,
    ...
    writer.write_all(
        b"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 ",
    )?;
...
pub fn decompress(
    ...
    Ok(decompress_with_dict_decompressor(
        &mut decompressor,
        dict_id,
        writer,
        archive,
...
pub fn get_tree(index_file: &mut (impl Read + Seek)) -> Result<FxHashMap<u32, u32>> {
    ...
    let mut map = FxHashMap::<u32, u32>::with_capacity_and_hasher(
        tree_item_len as usize,
